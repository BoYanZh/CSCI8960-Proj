...
DUMP_PATH = (
    "/checkpoint/%s/dumped"
    if "private" in __file__
    else "/mnt/vol/gfsai-east/ai-group/users/%s/dumped"
)
...
def accuracy_with_vote(preds, labels, K):
    ...
    preds = np.apply_along_axis(
        lambda x: np.bincount(x, minlength=1000), axis=1, arr=preds
    ).argmax(1)
    return (preds == labels).mean()
...
def get_optimizer(params, parameters, lr_init):
    ...
    if params.optimizer == "adadelta":
        ...
        raise Exception(
            'Unknown optimization params.optimizer: "%s"' % params.optimizer
        )
...
def reload_checkpoint(params, model, ema, optimizer, logger, checkpoint_path=""):
    ...
    """
    if checkpoint_path == "":
        checkpoint_path = params.dump_path
