...
def prepare_model(architecture):
    if "nf" in architecture:
        ...
        replace_conv_start = MyScaledStdConv2d(
            conv.in_channels,
            conv.out_channels,
        ...
        nn.init.kaiming_normal_(
            replace_conv_start.weight, mode="fan_in", nonlinearity="linear"
        )
        if replace_conv_start.bias is not None:
            nn.init.zeros_(replace_conv_start.bias)
...
def prepare_dataloaders(args):
    if args.transform:
        ...
        train_loader = get_data_loader(
            args, split="train", transform=args.train_transform, shuffle=True
        )

    test_loader = get_data_loader(
        args, split="valid", transform="center", shuffle=False
    )
...
def prepare_optimizer(weights, args):
    if args.optimizer == "AdamW":
        optimizer = optim.AdamW(
            weights, args.AdamW_lr, (args.AdamW_beta1, args.AdamW_beta1), args.AdamW_eps
        )
        ...
        optimizer = optim.SGD(
            weights, lr=args.lr, momentum=args.momentum, dampening=args.dampening
        )
    return optimizer
...
def prepare_augmult(model, transform):
    if transform:
        ...
        model.GRAD_SAMPLERS[nn.modules.linear.Linear] = (
            augmentation.augmented_compute_linear_grad_sample
        )
        model.GRAD_SAMPLERS[MyScaledStdConv2d] = (
            augmentation.augmented_compute_wsconv_grad_sample
        )
...
def prepare_augmult_cifar(model, transform):
    if transform:
        augmentation = AugmentationMultiplicity(transform)
        model.GRAD_SAMPLERS[torch.nn.modules.conv.Conv2d] = (
            augmentation.augmented_compute_conv_grad_sample
        )
        model.GRAD_SAMPLERS[torch.nn.modules.linear.Linear] = (
            augmentation.augmented_compute_linear_grad_sample
        )
        model.GRAD_SAMPLERS[nn.GroupNorm] = (
            augmentation.augmented_compute_group_norm_grad_sample
        )
