...
def compute_wsconv_grad_sample(
...
def compute_expand_grad_sample(layer, activations, backprops):
    """
...
class MyNormFreeBlock(nn.Module):
    ...
    def __init__(
        ...
        self.expand = Expand(torch.tensor(1.0)) if skipinit else None
    ...
    def forward(self, x):
        ...
        if self.expand is not None:
            out = out * self.expand(out).unsqueeze(1).unsqueeze(2).unsqueeze(
                3
            )  # carefull
