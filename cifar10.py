    ...
    with BatchMemoryManager(
        ...
        for i, (images, target) in enumerate(memory_safe_data_loader):
            ...
            if is_main_worker:
                ...
                per_sample_norms = (
                    torch.stack(per_param_norms, dim=1).norm(2, dim=1).cpu().tolist()
                )
            ...
            if is_updated:
                ...
                if is_main_worker:
                    ...
                    grad_sample_gradients_norms_per_epoch.append(
                        np.mean(grad_sample_norms)
                    )
                            ...
                            + json.dumps(
                                {
                                    ...
                                    "grad_sample_gradients_norms": np.mean(
                                        grad_sample_norms
                                    ),
...
def main():  ## for non poisson, divide bs by world size
    ...
    if is_main_worker:
        ...
        print(
            f"train_acc (EMA):{train_acc:.2f}, test_acc (EMA):{test_acc:.2f}, epsilon:{epsilons[-1]:.2f}"
        )
        logger.info(
            "__log:"
            + json.dumps(
...
def parse_args():
    ...
    parser.add_argument(
        "--experiment", default=0, type=int, help="seed for initializing training. "
    )
    parser.add_argument(
        "-c",
