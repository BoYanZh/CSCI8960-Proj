...
def train(
    ...
    with BatchMemoryManager(
        ...
        for images, target in memory_safe_data_loader:
            ...
            is_updated = not (
                optimizer._check_skip_next_step(pop_next=False)
            )  # check if we are at the end of a true batch without incrementing the count.
            ############

            #### LOGGING STATISTICS ABOUT THE GRADIENTS ###
            if is_main_worker:
                if nb_steps == 0 and is_updated:
                    logger.info(
                        "__log:"
                        + json.dumps(
                            {
                ...
                per_param_norms = torch.stack(
                    [
                        g.grad_sample.view(len(g.grad_sample), -1)
                        .norm(2, dim=-1)
                        .detach()
                        .cpu()
                        for g in model.parameters()
            ...
            if is_updated:
                ...
                update(
                    model, ema, nb_steps, decay=args.model_ema_decay
                )  ## Updating the EMA nodel
                if is_main_worker:
                            ...
                            + json.dumps(
                                {
                                    ...
                                    "current_momentum": optimizer.param_groups[0][
                                        "momentum"
                                    ],
                                    "nb_examples_since_last_update_main_worker": len(
                                        grad_sample_norms
                                    ),
                                    "grad_sample_gradients_norms_means": np.mean(
                                        grad_sample_norms
                                    ),
                                    "grad_sample_gradients_norms_lowerC": np.mean(
                                        np.array(grad_sample_norms)
                                        < args.max_per_sample_grad_norm
                                    ),
                                    "grad_sample_gradients_norms_medians": np.median(
                                        grad_sample_norms
                                    ),
                                    "grad_sample_gradients_norms_max": max(
                                        grad_sample_norms
                                    ),
                                    "grad_sample_gradients_norms_last_percentile": np.quantile(
                                        grad_sample_norms, 0.99
                                    ),
                                    # "grad_sample_gradients_norms_hist":list(np.histogram(grad_sample_norms,bins=np.arange(100), density=True)[0]),
                                    "per_param_norms": per_param_norms_epoch.mean(
                                        1
                                    ).tolist(),
...
def main():  ## for non poisson, divide bs by world size
    ...
    E = get_epochs_from_bs(
        args.batch_size, args.ref_nb_steps, len(train_loader.dataset)
    )
    if is_main_worker:
        print(
    ...
    if is_main_worker:
            ...
            + json.dumps(
                {
                    ...
                    "final_test_acc_ema_voted": test_acc_vote,
                }
            )
        )
        ...
        print(
            f"final test acc of non ema model:{test_acc:.2f}, final train acc of non ema model:{train_acc:.2f}"
        )
        logger.info(
            "__log:"
            + json.dumps(
                {
                    ...
                    "final_test_acc_non_ema_voted": test_acc_voted,
                }
            )
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="B",
        help=r"Batch size for simulated training. It will automatically set the noise $\sigma$ s.t. $B/\sigma = B_{ref}/sigma_{ref}$",
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=float,
        help="Training only on a subset of the training set. It will randomly select proportion training data",
    )
    parser.add_argument(
        "--lr",
        "--learning_rate",
    ...
    parser.add_argument(
        ...
        default=512,
        type=int,
        help="What batch size to use when evaluating the model the test set",
    )
    parser.add_argument(
        "--momentum",
        default=0,
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="seed for initializing training. Usefull when doing multiple experimemts in parrallel to get stds on the results",
    )
    ...
    parser.add_argument(
        ...
        default=8e-7,
        metavar="D",
        help="Delta for the DP accountant",
    )
    parser.add_argument(
        "--ref_B",
        type=int,
        default=32768,
        help="reference batch size used with reference noise and number of steps to create for the TAN sclaling stratedy",
    )
    parser.add_argument(
        "--ref_nb_steps",
        default=18000,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.5,
        help="reference noise used with reference batch size and number of steps",
    )
    parser.add_argument(
        "--poisson_sampling",
        type=bool_flag,
        default=True,
        help="using Poisson sampling - we only performed experiments when set to true because it is the only way to have valid DP guarantees",
    )
    ...
    parser.add_argument(
        "--train_path", type=str, default="", help="name of training set"
    )
    parser.add_argument(
        "--val_path",
        type=str,
    ...
    parser.add_argument(
        "--dump_path", type=str, default="", help="Where results will be stored"
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        "--freq_log", type=int, default=50, help="every each freq_log steps, we log."
    )
    parser.add_argument(
        "--freq_log_val",
        type=int,
        default=1000,
        help="every each freq_log steps, we evaluate the model on the validation set.",
    )
    parser.add_argument(
        "--starting_epoch",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Order of the Augmentation multplicity (AugMult). If non 0, each image in each batch will be duplicated 'transform' times, and randomly applied 'type_of_augmentation'.",
    )
    parser.add_argument(
        "--type_of_augmentation",
        default="OursBest",
        help="type of AugMult that will be used if 'args.transform' is non 0.",
    )
    parser.add_argument(
        "--train_transform",
        choices=["random", "flip", "center", "simclr", "beit", "resize"],
        default="center",
        help="equivalent to AugMult of order 1, i.e classic data augmentation.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="nf_resnet50",
        help="Type of architecture of the network.",
    )
